(() => {
  if (window.__choppaMidiCaptureV26) return;
  window.__choppaMidiCaptureV26 = true;
  ...
  const STORAGE_KEY = 'choppa.launchkey.capture.v26';
  const IGNORED_REALTIME = new Set([248, 254]);
    ...
    panel.innerHTML = `
      <div class="midiCaptureHead">
        <div>
          ...
          <p>Capture only. MIDI clock <b>248</b> and active sensing <b>254</b> are ignored, because clocks are not buttons despite their desperate need for attention.</p>
        </div>
      ...
      <div class="midiCaptureBody">
        ...
        <div class="midiCaptureScreen">
          ...
          <div id="midiCaptureHelp" class="midiCaptureSub">Click Start Capture, select a control, then press the matching Launchkey control once.</div>
  ...
  function status(text, kind = '') {
    ...
    if (!el) return;
    el.className = 'midiCaptureStatus ' + kind;
    el.innerHTML = '<i></i> ' + text;
  }

  function isIgnoredClock(data) {
    return data.length === 1 && IGNORED_REALTIME.has(data[0]);
  }

  function isCaptureWorthy(data) {
    if (!data.length || isIgnoredClock(data)) return false;
    const st = data[0];
    const cmd = st & 240;
    if (cmd === 144 && (data[2] || 0) === 0) return false;
    if (cmd === 128) return false;
    return true;
  }
  ...
  function onMidi(ev) {
    ...
    if (isIgnoredClock(data)) {
      ignoredCount++;
      if (ignoredCount === 1 || ignoredCount % 96 === 0) status('Ignoring MIDI clock ' + ignoredCount, '');
      return;
    }
    if (!isCaptureWorthy(data) || !activeTarget) return;
    ...
    saveCaptures();
    ignoredCount = 0;
    status('Captured ' + labelFor(activeTarget), 'on');
    render();
    nextEmpty(false);
  }
  ...
  function render() {
    ...
    if (box) {
      ...
      box.querySelectorAll('[data-target]').forEach(btn => btn.addEventListener('click', () => { activeTarget = btn.dataset.target; render(); }));
    }

    const rec = captures[activeTarget];
    if ($('midiCaptureName')) $('midiCaptureName').textContent = labelFor(activeTarget);
    if ($('midiCaptureHelp')) $('midiCaptureHelp').textContent = rec ? 'Captured. Press the control again to replace it, or move to the next empty slot.' : 'Waiting for this Launchkey control. Clock 248 is ignored.';
  ...
  function attachInputs() {
    ...
    midiAccess.inputs.forEach(input => {
      count++;
      if (input.__choppaCaptureV26) return;
      input.__choppaCaptureV26 = true;
      input.addEventListener('midimessage', onMidi);
    });
    status(count ? 'Capturing ' + count + ' Input' + (count === 1 ? '' : 's') : 'No Inputs', count ? 'on' : 'bad');
  }

  async function start() {
    if (armed) { attachInputs(); return; }
    if (!navigator.requestMIDIAccess) {
      status('MIDI Unsupported', 'bad');
      if ($('midiCaptureHelp')) $('midiCaptureHelp').textContent = 'This browser does not support Web MIDI.';
      return;
    }
    try {
      status('Requesting MIDI', '');
      midiAccess = await navigator.requestMIDIAccess();
      armed = true;
      attachInputs();
      midiAccess.addEventListener?.('statechange', attachInputs);
      if ($('midiCaptureHelp')) $('midiCaptureHelp').textContent = 'Capture is armed. MIDI clock 248 will be ignored.';
    } catch (err) {
      status('MIDI Blocked', 'bad');
  ...
  async function copyJson() {
    ...
    try { await navigator.clipboard.writeText(text); status('Copied JSON', 'on'); }
    catch { status('Copy Failed', 'bad'); const area = $('midiCaptureExport'); if (area) area.select(); }
  }
  ...
  function clearCaptures() {
    ...
    ignoredCount = 0;
    render();
    status(armed ? 'Capturing' : 'Capture Off', armed ? 'on' : '');
  }
